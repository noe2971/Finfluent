...
const Information = () => {
  ...
  // This flag ensures we only check Firebase once on mount.
  ...
  const fetchStockInfo = async (forceRefresh = false) => {
    ...
    // Only check Firebase cache once.
  ...
  // Auto-check Firebase and fetch analysis only once.
  useEffect(() => {
    if (stockSymbol && userId && profileData && !hasCheckedFirebase) {
      fetchStockInfo();
    }
  }, [stockSymbol, userId, profileData, hasCheckedFirebase]);
